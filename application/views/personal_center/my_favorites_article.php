...
<html>
    ...
    <body id="my_favorites_article">
        ...
        <form action="/personal_center/my_favorites_article_del" method="post">
            <ul class="list">
                <?php if (!empty($favorites_article_list)): ?>
                    <?php foreach ($favorites_article_list as $fal): ?>
                        <li>
                            <div class="left">
                                <input name="list_id[]" type="checkbox"  value="<?= $fal['id'] ?>" />
                            </div>
                            <div class="right">
                                <a class="article_name" href="/article/detail?article_id=<?= $fal['article_id'] ?>"><?= $fal['title'] ?></a>
                                <div class="clear"></div>
                                <span class="read">阅读<?= $fal['read'] ?></span>
                                <span class="time"><?= date('Y-m-d', $fal['addtime']) ?></span>
                            </div>
                        </li>
                    <?php endforeach; ?>
                <?php else: ?>
                    <div id="empty_box">
                        <i></i>
                        <p>目前没有收藏的简章！<a href="/article">去收藏简章</a></p>
                    </div>
                <?php endif; ?>
            </ul>
            <?php if ($page_arr['totalpage'] > 1): ?>
                <div class="clear"></div>
                <div id="page_box" data-url="/personal_center/my_favorites_article/" data-parameter="">
