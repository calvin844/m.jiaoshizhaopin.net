...
<html>
    ...
    <body id="my_apply_jobs">
        ...
        <div class="clear"></div>
        <ul class="list">
            <?php if (!empty($apply_jobs_list)): ?>
                <?php foreach ($apply_jobs_list as $ajl): ?>
                    <li>
                        <a class="job_name" href="/job/detail?job_id=<?= $ajl['jobs_id'] ?>"><?= $ajl['jobs_name'] ?></a>
                        <span class="time"><?= date('Y-m-d', $ajl['apply_addtime']) ?></span>
                        <p class="p_name"><?= $ajl['company_name'] ?></p>
                        <?php if ($ajl['personal_look'] != 1): ?><i></i><?php endif; ?>
                    </li>
                <?php endforeach; ?>
            <?php else: ?>
                <div id="empty_box">
                    <i></i>
                    <p>目前没有已申请的职位！<a href="/job">去申请职位</a></p>
                </div>
            <?php endif; ?>
        </ul>
        <script>
        ...
        <?php if ($page_arr['totalpage'] > 1): ?>
            <div class="clear"></div>
            <div id="page_box" data-url="/personal_center/my_apply_jobs/" data-parameter="">
