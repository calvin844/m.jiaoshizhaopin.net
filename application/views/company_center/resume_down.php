...
<html>
    ...
    <body id="download">
        ...
        <form id="down_form" action="" method="post">
            <ul class="list">
                <?php foreach ($down_list as $dl): ?>
                    <li data-url="/resume/detail?resume_id=<?= $dl['resume_id'] ?>">
                        <div class="left">
                            <input type="checkbox" name="did[]" value="<?= $dl['did'] ?>" />
                        </div>
                        <div class="right">
                            <?php if (!empty($dl['fullname'])): ?>
                                <div class="header_box">
                                    <img src="/data/photo/<?= $dl['photo_img'] ?>" width="40" height="40" />
                                </div>
                                <div class="info_box">
                                    <div class="top">
                                        <p class="name"><?= $dl['fullname'] ?></p>
                                        <span class="time"><?= date("Y/m/d H:i:s", $dl['down_addtime']) ?></span>
                                    </div>
                                    <div class="bottom">
                                        <?php if ($dl['default_resume'] == 0): ?>
                                            <p class="info"><?= $dl['education_cn'] ?>&nbsp;|&nbsp;<?= $dl['district_cn'] ?>&nbsp;|&nbsp;<?= $dl['experience_cn'] ?></p>
                                        <?php else: ?>
        ...
        <?php if ($page_arr['totalpage'] > 1): ?>
            <div class="clear"></div>
            <div id="page_box" data-url="/company_center/resume_down/" data-parameter="">
        ...
        <script>
            ...
            $('#collect').click(function() {
                ...
                $('#down_form').attr('action', '/company_center/resume_to_collect');
                $('#down_form').submit();
            })
            $('#delete').click(function() {
                $('#check_all').prop("checked", false);
                $('#down_form').attr('action', '/company_center/del_down');
                $('#down_form').submit();
            })
        </script>
        <div class="clear"></div>
