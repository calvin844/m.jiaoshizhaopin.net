...
<html>
    ...
    <body id="company_jobs">
        ...
        <form id="audit_form" action="" method="post">
            <ul class="list">
                <?php foreach ($audit_list as $al): ?>
                    <li data-url="/job/detail?job_id=<?= $al['id'] ?>">
                        <div class="left">
                            <input type="checkbox" name="did[]" value="<?= $al['id'] ?>" />
                        </div>
                        <div class="right">
                            <p class="job_name"><?= $al['jobs_name'] ?></p>
                            <div class="clear"></div>
                            <span class="date"><?= date("Y/m/d", $al['refreshtime']) ?></span>
                        </div>
                    </li>
                <?php endforeach; ?>
            </ul>
        ...
        <?php if ($page_arr['totalpage'] > 1): ?>
            <div class="clear"></div>
            <div id="page_box" data-url="/company_center/jobs_audit/" data-parameter="">
        ...
        <script>
            ...
            $('#delete').click(function() {
                ...
                $('#audit_form').attr('action', '/company_center/job_to_delete');
                $('#audit_form').submit();
            })
        </script>
        <div class="clear"></div>
