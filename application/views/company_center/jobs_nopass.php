...
<html>
    ...
    <body id="company_jobs">
        ...
        <div class="clear"></div>
        <form id="nopass_form" action="" method="post">
            <ul class="list">
                <?php foreach ($nopass_list as $nl): ?>
                    <li data-url="/job/detail?job_id=<?= $nl['id'] ?>">
                        <div class="left">
                            <input type="checkbox" name="did[]" value="<?= $nl['id'] ?>" />
                        </div>
                        <div class="right">
                            <p class="job_name"><?= $nl['jobs_name'] ?></p>
                            <div class="clear"></div>
                            <p class="result"><?= $nl['reason'] ?></p>
                            <div class="clear"></div>
                            <span class="date"><?= date("Y/m/d", $nl['refreshtime']) ?></span>
                        </div>
                    </li>
                <?php endforeach; ?>
            </ul>
        ...
        <?php if ($page_arr['totalpage'] > 1): ?>
            <div class="clear"></div>
            <div id="page_box" data-url="/company_center/jobs_nopass/" data-parameter="">
        ...
        <script>
            ...
            $('#delete').click(function() {
                ...
                $('#nopass_form').attr('action', '/company_center/job_to_delete');
                $('#nopass_form').submit();
            })
        </script>
        <div class="clear"></div>
