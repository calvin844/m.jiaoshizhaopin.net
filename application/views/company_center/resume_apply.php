...
<html>
    ...
    <body id="apply_jobs">
        ...
        <div class="clear"></div>
        <form id="apply_form" action="" method="post">
            <ul class="list">
                <?php foreach ($apply_list as $al): ?>
                    <li data-url="/resume/detail?apply_id=<?= $al['did'] ?>">
                        <div class="left">
                            <input type="checkbox" name="did[]" value="<?= $al['did'] ?>" />
                        </div>
                        <div class="right">
                            <div class="header_box">
                                <img src="/data/photo/<?= $al['photo_img'] ?>" width="40" height="40" />
                                <?php if ($al['personal_look'] == 1): ?>
                                    <div class="no_look"></div>
                                <?php endif; ?>
                            </div>
                            <div class="info_box">
                                <div class="top">
                                    <p class="name"><?= $al['fullname'] ?></p>
                                    <span class="time"><?= date('Y/m/d H:i:s', $al['apply_addtime']) ?></span>
                                </div>
                                <div class="clear"></div>
                                <div class="bottom">
                                    <p class="job"><?= $al['jobs_name'] ?></p>
                                </div>
                            </div>
                        </div>
                    </li>
            ...
            <input type="hidden" name="set_look" value="2" />
        </form>
        <div class="clear"></div>
        <?php if ($page_arr['totalpage'] > 1): ?>
            <div class="clear"></div>
            <div id="page_box" data-url="/company_center/resume_apply/" data-parameter="">
        ...
        <script>
            ...
            $('#set_look').click(function() {
                ...
                $('#apply_form').attr('action', '/company_center/resume_set_look');
                $('#apply_form').submit();
            })
            $('#delete').click(function() {
                $('#check_all').prop("checked", false);
                $('#apply_form').attr('action', '/company_center/del_apply');
                $('#apply_form').submit();
            })
        </script>
        <div class="clear"></div>
