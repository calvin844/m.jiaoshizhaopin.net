...
<html>
    ...
    <body id="company_jobs">
        ...
        <form id="release_form" action="" method="post">
            <ul class="list">
                <?php foreach ($release_list as $rl): ?>
                    <li data-url="/job/detail?job_id=<?= $rl['id'] ?>">
                        <div class="left">
                            <input type="checkbox" name="did[]" value="<?= $rl['id'] ?>" />
                        </div>
                        <div class="right">
                            <p class="job_name"><?= $rl['jobs_name'] ?></p>
                            <div class="clear"></div>
                            <p class="info">应聘：<?= $rl['resume_total'] ?> | 浏览：<?= $rl['click'] ?></p>
                            <span class="date"><?= date("Y/m/d", $rl['refreshtime']) ?></span>
                        </div>
                    </li>
                <?php endforeach; ?>
            </ul>
        ...
        <?php if ($page_arr['totalpage'] > 1): ?>
            <div class="clear"></div>
            <div id="page_box" data-url="/company_center/jobs_release/" data-parameter="">
        ...
        <div class="all_box">
            ...
            <div class="right">
                <input id="stop" class="button" type="button" value="暂停" />
                <input id="refresh" class="button" type="button" value="刷新" />
        ...
        <script>
            ...
            $('#stop').click(function() {
                ...
                $('#release_form').attr('action', '/company_center/job_to_stop/2');
                $('#release_form').submit();
            })
            $('#refresh').click(function() {
                $('#check_all').prop("checked", false);
                $('#release_form').attr('action', '/company_center/job_to_refresh');
                $('#release_form').submit();
            })
            $('#delete').click(function() {
                $('#check_all').prop("checked", false);
                $('#release_form').attr('action', '/company_center/job_to_delete');
                $('#release_form').submit();
            })
        </script>
        <div class="clear"></div>
